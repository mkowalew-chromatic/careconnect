...
`@careconnect/design-system` versions independently of the app workspaces. To
fold it into their unified version instead, add it to `fixed` in
...
The repo root `.nvmrc` pins the Node version, and the workflows read it via
`node-version-file`, so local and CI cannot drift. It must stay at 22.22.2 or
newer — `jsdom` and `undici` in the test toolchain require it, and older Node
fails with `webidl.util.markAsUncloneable is not a function`.
